...
POSSIBLE_PATHS = [
    "loan_dataset_raw/bank_transactions_data.csv",    
    "../loan_dataset_raw/bank_transactions_data.csv", 
    "data/loan_dataset_raw/bank_transactions_data.csv"
]
...
def load_data():
    if RAW_PATH is None:
        ...
        print(f"Isi folder saat ini: {os.listdir(os.getcwd())}")
        raise FileNotFoundError("Dataset tidak ditemukan")
            
    df = pd.read_csv(RAW_PATH)
    print(f"Dataset Loaded from {RAW_PATH}: {df.shape}")
    return df
...
def save_result(X_pca):
    ...
    print(f"File BERHASIL disimpan di: {os.path.abspath(OUT_FILE)}")
